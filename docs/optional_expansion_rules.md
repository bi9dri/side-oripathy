...
追加ルールにさらなるルールも追加できます。何を言っているんだと思うかもしれませんが、アークナイツの世界観をより詳細に表現するために、ここまでの追加ルールを拡張する方法の一例です。よければ参考にしてみてください。

## 選択ルール【サルカズ傭兵】

種族が「サルカズ」の共鳴者を作成するとき、以下のすべての効果をまとめて反映します（一部のみを選ぶのは不可）。

* 侵食度の初期値が21以上の任意の数値になる。
* 能力値に関わらず、生理的耐性が2になる。
...
エモクロアTRPGの共鳴システムによって、「シーボーン」への同化を表現します。〈∞共鳴〉が10になったとき、共鳴者はシーボーンへと成り果て、ロストします。

また、種族が「アビサル」の共鳴者を作成するとき、以下のすべての効果を反映します。

* 能力値に関わらず、生理的耐性が12になる。
* 〈アーツ〉技能を取得できない。
* 〈∞共鳴〉の初期レベルが3以上の任意の数値になる。
* シーボーンへの共鳴判定が常に完全一致になる。
  * 〈∞共鳴〉が10未満であっても、極限共鳴（ハウリング）などによってシーボーンの味方になる可能性がある。
* 侵食段階によるすべての影響を無効化する。
  * 発症自体はしているものの、判定に影響を及ぼすほどではないという解釈。
